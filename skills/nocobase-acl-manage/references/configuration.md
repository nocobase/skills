...
All checks above should be executed through resolved CLI runtime commands via skill-local wrapper (`node ./run-ctl.mjs -- <nocobase-ctl-args>`) and command help discovery, with env context resolved first by `$nocobase-env-bootstrap task=app-manage app_env_action=current app_scope=project target_dir=<target_dir>`.
