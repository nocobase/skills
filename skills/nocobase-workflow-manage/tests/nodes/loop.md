# Loop (loop) Node Tests

## Overview
Tests for the `loop` node type.

## Test Cases

### Creation Scenarios

#### TC-NODE-LOOP-001: Add Loop node
- **Description**: Add Loop node to workflow
- **Prompt**: "增加循环节点，循环订单明细"
- **Expected Configuration**:
```json
{
  // Configuration based on loop documentation
}
```
- **Validation Points**:
  - Node type should be `loop`
  - Configuration should match expected structure
- **Test Steps**:
  1. Create workflow with appropriate trigger
  2. Execute skill with the prompt
  3. Verify node added with correct type
  4. Test node functionality

### Editing Scenarios

#### TC-NODE-LOOP-002: Modify existing Loop node
- **Description**: Update configuration of existing Loop node
- **Prompt**: "修改Loop节点的配置"
- **Expected Configuration** (updated):
```json
{
  // Updated configuration
}
```
- **Validation Points**:
  - Configuration should be updated
  - Node type unchanged
- **Test Steps**:
  1. Create workflow with loop node
  2. Execute skill with edit prompt
  3. Verify configuration updated
  4. Test updated functionality

## Test Data Requirements
- Appropriate collections and data for testing
- Workflow context matching node requirements

## Notes
Refer to [loop.md](loop.md) for detailed configuration options.
