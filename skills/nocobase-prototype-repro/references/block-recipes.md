...
| **Gantt — standard start/end timeline** (campaign / project schedules) | **native `GanttBlockModel` FIRST** — NocoBase ships one; bind it whenever the visual is plain date-range bars per row | native component beats a hand-drawn JS gantt. ⚠️ **Failure seen (room booking): agent ships a plain CRUD table of rooms.** A schedule / availability / timeline board is **never** a table — if it isn't the native Gantt, it's the time-lane below. The lanes ARE the signature. |
...
| **Rich kanban card** | native Kanban (drag preserved) + ONE JS card field (limit is 2 fields → remove natives first) | card click passes through to native `KanbanCardViewActionModel` as long as the JS body doesn't `stopPropagation`. ⚠️ **Failure seen (helpdesk): columns + counts correct but every card renders "Untitled" / blank.** That means the JS card field was never added (or no title mapped). You MUST add the JS card field, render real content (priority badge + title + assignee avatar), and **verify one card shows data before done** — a right-looking board with empty cards is a fail. |
| **Occupancy / bin-grid heatmap** (warehouse rack map, seat map, status wall) | JSBlock rendering a CSS-grid of cells, each colored by a per-cell count / status; aggregate once via `ctx.api.request` grouped by the cell key (zone+slot) | ⚠️ **Failure seen (warehouse): the grid renders but every cell is 0 / empty** — the per-cell aggregation returned nothing because the group key didn't match the seed. Seed the bins/slots collection densely and confirm the group-by returns rows BEFORE wiring the grid. Match the prototype's dark "command-center" theme via `$p.__theme` when it has one. |
| **Compact record-card panel** ("currently checked out", "recent activity", contact rows) | List + JSField rendering a **tight one/two-line row card** (avatar + 2-3 inline fields), small List `pageSize` | ⚠️ **Failure seen (asset tracking): each record rendered as a tall vertical stack of label/value rows → an unusably long page.** Do NOT dump every field vertically. Compact horizontal card (name + status pill + one meta line), cap `pageSize` (~6-8), scope panel height. |
...
| **Stage-progress / checklist journey** (onboarding, pipeline stages with nested items) | List + JSField, each row a stage card with a horizontal segment bar + its nested checklist items | same wrapper-uncollapse CSS as §3. ⚠️ **Failure seen (onboarding): the page shows "No records found".** The List resource wasn't bound to the stages collection, or the parent (the hire) wasn't seeded. Seed ≥1 parent **with** children and confirm the List returns rows before rendering the journey. |
