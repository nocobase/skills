/**
 * KPI Card Template — config-driven
 */
...
const CONFIG = {
  ...
  format: 'percent',
  dataSourceKey: 'main',
  reportUid: 'helpdesk_kpi_sla_compliance',
  sql: `SELECT COALESCE(ROUND((COUNT(*) FILTER (WHERE "status" IN ('Resolved', 'Closed')) * 100.0 / NULLIF(COUNT(*), 0)), 1), 100) AS value FROM "nb_helpdesk_tickets" WHERE "createdAt" > NOW() - '30 days'::interval`
};
// ==================== END CONFIG ====================
...
const cardStyle = (color, bgColor) => ({
  ...
  display: 'flex', flexDirection: 'column', cursor: CONFIG.popup ? 'pointer' : 'default'
});
...
const KpiCard = () => {
  const [data, setData] = useState({ value: 0, loading: true });
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const r = result?.[0] || {};
        setData({ value: r.value || 0, loading: false });
  ...
  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle(CONFIG.color, CONFIG.bgColor) },
    ctx.React.createElement('div', { style: labelStyle }, CONFIG.label),
