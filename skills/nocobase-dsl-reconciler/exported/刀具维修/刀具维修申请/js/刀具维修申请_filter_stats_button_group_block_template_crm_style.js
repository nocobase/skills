...
const TARGET_BLOCK_UID = 'e2ez17s5n5t';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'nb_tm_repair';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 's2', label: '维修中', filter: {"status": {"$eq": "维修中"}} },
      { key: 's3', label: '待验收', filter: {"status": {"$eq": "待验收"}} },
      { key: 's4', label: '已完成', filter: {"status": {"$eq": "已完成"}} }
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
