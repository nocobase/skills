...
const TARGET_BLOCK_UID = 'vw2o9qyu14s';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'nb_tm_tools';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 's1', label: '在库', filter: {"status": {"$eq": "在库"}} },
      { key: 's2', label: '使用中', filter: {"status": {"$eq": "使用中"}} },
      { key: 's3', label: '维修中', filter: {"status": {"$eq": "维修中"}} },
      { key: 's4', label: '保养中', filter: {"status": {"$eq": "保养中"}} },
      { key: 's5', label: '已报废', filter: {"status": {"$eq": "已报废"}}, danger: true }
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
