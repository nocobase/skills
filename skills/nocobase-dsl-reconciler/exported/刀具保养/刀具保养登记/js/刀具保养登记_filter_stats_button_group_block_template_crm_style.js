...
const TARGET_BLOCK_UID = 'a0ehi2l633v';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'nb_tm_maintenance';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 's1', label: '待执行', filter: {"status": {"$eq": "待执行"}} },
      { key: 's2', label: '已执行', filter: {"status": {"$eq": "已执行"}} },
      { key: 's3', label: '主管已审', filter: {"status": {"$eq": "主管已审"}} }
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
