...
const TARGET_BLOCK_UID = 'y4q5s6u4zzp';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'nb_tm_machines';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 's1', label: '运行中', filter: {"status": {"$eq": "运行中"}} },
      { key: 's2', label: '待机', filter: {"status": {"$eq": "待机"}} },
      { key: 's3', label: '维护中', filter: {"status": {"$eq": "维护中"}} },
      { key: 's4', label: '停用', filter: {"status": {"$eq": "停用"}}, danger: true }
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
