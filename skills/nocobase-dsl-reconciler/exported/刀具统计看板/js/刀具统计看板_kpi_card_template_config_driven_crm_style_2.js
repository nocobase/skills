/**
 * KPI Card Template — config-driven, CRM-style
 *
...
const CONFIG = {
  ...
  format: 'number',
  dataSourceKey: 'main',
  reportUid: 'tm_kpi_instock',
  sql: `
SELECT COUNT(*) AS value,
  ROUND(COUNT(*)::numeric * 100 / NULLIF((SELECT COUNT(*) FROM nb_tm_tools), 0), 0) || '% 占比' AS sub_text
FROM nb_tm_tools WHERE status = '在库'
`,
  // popup: { title: 'Details', mode: 'drawer', size: 'large' },
};
// ==================== END CONFIG ====================
...
const cardStyle = (color, bgColor) => ({
  ...
  display: 'flex', flexDirection: 'column', cursor: CONFIG.popup ? 'pointer' : 'default', overflow: 'hidden'
});
...
const KpiCard = () => {
  ...
  const handleClick = CONFIG.popup ? async () => {
    await ctx.openView(ctx.model.uid + '-detail', {
      mode: CONFIG.popup.mode || 'dialog',
      title: CONFIG.popup.title || CONFIG.label,
      size: CONFIG.popup.size || 'small',
    });
  } : undefined;

  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle(CONFIG.color, CONFIG.bgColor), onClick: handleClick },
    ...
    ctx.React.createElement('svg', { style: bgChartStyle, viewBox:'0 0 100 50', preserveAspectRatio:'none' },
      ctx.React.createElement('path', { d:'M0,50 L0,30 Q25,10 50,25 T100,15 L100,50 Z', fill: CONFIG.bgColor, stroke: CONFIG.strokeColor, strokeWidth:'2' }))
  );
};
