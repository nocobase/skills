...
const TARGET_BLOCK_UID = '9z4txwih30c';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'nb_tm_scrap';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 's1', label: '待审批', filter: {"status": {"$eq": "待审批"}} },
      { key: 's2', label: '技术部已批', filter: {"status": {"$eq": "技术部已批"}} },
      { key: 's3', label: '已报废', filter: {"status": {"$eq": "已报废"}} },
      { key: 's4', label: '已驳回', filter: {"status": {"$eq": "已驳回"}}, danger: true }
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
