...
const SQL_UID = 'erp_sales_avg_order_value_kpi_v3';
const SQL = `
WITH current_period AS (
  SELECT COALESCE(AVG(total_amount), 0) AS current_value
  FROM nb_erp_customer_orders
  WHERE status <> 'cancelled'
    AND order_date >= {{startDate1}} AND order_date <= {{endDate1}}
),
previous_period AS (
  SELECT COALESCE(AVG(total_amount), 0) AS previous_value
  FROM nb_erp_customer_orders
  WHERE status <> 'cancelled'
...
const KpiCard = () => {
  ...
  const handleClick = () => {
    ctx.router.navigate('/admin/erp/orders');
  };

  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle('#7c3aed', '#f5f3ff'), onClick: handleClick },
    ctx.React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 2 } },
      ctx.React.createElement('span', { style: labelStyle }, t('Avg Order Value')),
      ctx.React.createElement('span', { style: trendPillStyle('#7c3aed', '#ede9fe') }, data.growthRate >= 0 ? `+${data.growthRate.toFixed(1)}%` : `${data.growthRate.toFixed(1)}%`)
    ),
    ctx.React.createElement('div', { style: valueStyle('#7c3aed') }, data.loading ? '...' : fmt(data.value)),
    ctx.React.createElement('svg', { style: bgChartStyle, viewBox: '0 0 100 50', preserveAspectRatio: 'none' },
      ctx.React.createElement('path', { d: 'M0,50 L0,30 Q25,10 50,25 T100,15 L100,50 Z', fill: '#ede9fe', stroke: '#c4b5fd', strokeWidth: '2' }))
  );
};
