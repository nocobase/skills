...
const SQL_UID = 'erp_dashboard_open_orders_kpi_v3';
const SQL = `
...
const KpiCard = () => {
  ...
  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle('#1d4ed8', '#eff6ff'), onClick: handleClick },
    ctx.React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 2 } },
      ctx.React.createElement('span', { style: labelStyle }, t('Open Orders')),
      ctx.React.createElement('span', { style: trendPillStyle('#1d4ed8', '#dbeafe') }, data.growthCount >= 0 ? `+${data.growthCount}` : `${data.growthCount}`)
    ),
    ctx.React.createElement('div', { style: valueStyle('#1d4ed8') }, data.loading ? '...' : data.count.toLocaleString()),
    ctx.React.createElement('svg', { style: bgChartStyle, viewBox: '0 0 100 50', preserveAspectRatio: 'none' },
      ctx.React.createElement('path', { d: 'M0,50 L0,40 L20,35 L40,20 L60,30 L80,10 L100,5 L100,50 Z', fill: '#dbeafe', stroke: '#93c5fd', strokeWidth: '2' }))
  );
};
