...
const SQL_UID = 'erp_dashboard_inventory_value_kpi_v3';
const SQL = `
WITH current_period AS (
  SELECT COALESCE(SUM(current_stock * standard_cost), 0) AS current_value
  FROM nb_erp_products
),
previous_period AS (
  SELECT COALESCE(SUM(
    CASE
      WHEN COALESCE(last_restock_date::timestamp, "createdAt") < {{startDate1}}
      THEN current_stock * standard_cost
      ELSE 0
    END
  ), 0) AS previous_value
  FROM nb_erp_products
...
const KpiCard = () => {
  ...
  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle('#b45309', '#fff7ed'), onClick: handleClick },
    ctx.React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 2 } },
      ctx.React.createElement('span', { style: labelStyle }, t('Inventory Value')),
      ctx.React.createElement('span', { style: trendPillStyle('#b45309', '#ffedd5') }, data.growthRate >= 0 ? `+${data.growthRate.toFixed(1)}%` : `${data.growthRate.toFixed(1)}%`)
    ),
    ctx.React.createElement('div', { style: valueStyle('#b45309') }, data.loading ? '...' : fmt(data.value)),
    ctx.React.createElement('svg', { style: bgChartStyle, viewBox: '0 0 100 50', preserveAspectRatio: 'none' },
      ctx.React.createElement('path', { d: 'M0,50 L0,30 Q25,10 50,25 T100,15 L100,50 Z', fill: '#ffedd5', stroke: '#fdba74', strokeWidth: '2' }))
  );
};
