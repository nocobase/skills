...
const SQL_UID = 'erp_dashboard_total_sales_kpi_v3';
const SQL = `
WITH current_period AS (
  SELECT COALESCE(SUM(total_amount), 0) AS current_value
  FROM nb_erp_customer_orders
  WHERE status IN ('confirmed', 'picking', 'shipped', 'completed')
...
const KpiCard = () => {
  ...
  return ctx.React.createElement('div', { className: 'kpi-card-hover', style: cardStyle('#0f766e', '#ecfeff'), onClick: handleClick },
    ctx.React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 2 } },
      ctx.React.createElement('span', { style: labelStyle }, t('Total Sales')),
      ctx.React.createElement('span', { style: trendPillStyle('#0f766e', '#ccfbf1') }, data.growthRate >= 0 ? `+${data.growthRate.toFixed(1)}%` : `${data.growthRate.toFixed(1)}%`)
    ),
    ctx.React.createElement('div', { style: valueStyle('#0f766e') }, data.loading ? '...' : fmt(data.value)),
