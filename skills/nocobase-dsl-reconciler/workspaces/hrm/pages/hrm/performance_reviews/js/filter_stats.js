/**
 * Performance review stats filter block
...
const CONFIG = {
  resource: 'nb_hrm_performance_reviews:list',
  stats: [
    { key: 'all', label: 'All', filter: null },
    { key: 'draft', label: 'Draft', filter: { status: { $eq: 'draft' } } },
    { key: 'submitted', label: 'Submitted', filter: { status: { $eq: 'submitted' } } },
    { key: 'completed', label: 'Completed', filter: { status: { $eq: 'completed' } } },
    { key: 'archived', label: 'Archived', filter: { status: { $eq: 'archived' } } },
  ],
};
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        console.error('Performance review stats fetch failed:', e);
      }
      setLoading(false);
    };
    fetchCounts();
