...
const CONFIG = {
  resource: 'nb_hrm_payroll:list',
  stats: [
    { key: 'all', label: 'All', filter: null },
    { key: 'pending', label: 'Pending', filter: { payment_status: { $eq: 'pending' } } },
    { key: 'processed', label: 'Processed', filter: { payment_status: { $eq: 'processed' } } },
    { key: 'paid', label: 'Paid', filter: { payment_status: { $eq: 'paid' } } },
    { key: 'on_hold', label: 'On Hold', filter: { payment_status: { $eq: 'on_hold' } } },
  ],
};
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        console.error('Payroll stats fetch failed:', e);
      }
      setLoading(false);
    };
    fetchCounts();
