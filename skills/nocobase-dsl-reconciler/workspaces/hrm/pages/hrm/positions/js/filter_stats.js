...
const CONFIG = {
  resource: 'nb_hrm_positions:list',
  stats: [
    { key: 'all', label: 'All', filter: null },
    { key: 'open', label: 'Open', filter: { status: { $eq: 'open' } } },
    { key: 'filled', label: 'Filled', filter: { status: { $eq: 'filled' } } },
    { key: 'frozen', label: 'Frozen', filter: { status: { $eq: 'frozen' } } },
  ],
};
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        ...
        console.error('Positions stats fetch failed:', e);
      }
      setLoading(false);
    };
    fetchCounts();
