...
const STATS = [
  ...
  { key: 'pending', label: 'Pending', filter: { status: { $eq: 'pending' } }, group: 'status' },
  { key: 'approved', label: 'Approved', filter: { status: { $eq: 'approved' } }, group: 'status' },
  { key: 'rejected', label: 'Rejected', filter: { status: { $eq: 'rejected' } }, group: 'status' },
];
      ...
      try {
        ...
        const results = await Promise.all(
          statusStats.map(s => ctx.api.request({
            url: 'nb_hrm_leave_requests:list',
