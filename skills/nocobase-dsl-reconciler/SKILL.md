...
description: >-
  **Opt-in DSL path** for NocoBase app building. Use ONLY when the user
  ...
  files ‚Ä?e.g. "use the DSL reconciler", "I want YAML I can commit",
  ...
  `nocobase-ui-builder` instead ‚Ä?this reconciler is still in active
...
`nocobase-ui-builder` instead ‚Ä?it's the default entry point.
...
workflows into your project ‚Ä?you then spend the whole session fighting
...
it says rather than guessing ‚Ä?don't grep through `src/deploy/*.ts`.

## When NOT to use this skill

Hand off to the matching skill when the user's request is orthogonal:

| User asks for‚Ä?| Skill |
|---|---|
| One-off live-UI tweak on an already-running page (move / reorder / reconfigure a single block, field, action) ‚Ä?**no DSL commit wanted** | `nocobase-ui-builder` |
...
`workspaces/<project>/` ‚Ä?stays here.
...
# valid ‚Ä?--copy is for your *extensions* before popups are wired.
npx tsx cli/cli.ts push <name> --force
```

The starter is a complete minimal CRUD ‚Ä?1 collection (Projects),
1 Dashboard page (4 KPI tiles + 2 charts), 1 list page with
filterForm / table / addNew popup / detail popup / 2 updateRecord
recordActions. Push as-is ‚Ü?visible in NB ‚Ü?then edit.
...
1. **Rename identifiers** to match the user's domain.
   - `collections/nb_starter_projects.yaml` ‚Ü?your collection name
...
builds ‚Ä?land the skeleton, show the user, gather feedback, iterate.
...
stage ‚Ä?validator would fire errors about "m2o field X has no popup
...
## Incremental edits ‚Ä?existing workspace

- **Add** a block / field / action / popup ‚Ü?write the DSL ‚Ü?push.
- **Remove** from DSL ‚Ü?push. The reconciler destroys the matching
  live model on the NB side and cleans `state.yaml`. Manual NB-UI
  authored elements (not tracked in `state.yaml`) are left alone.
- **Rename** ‚Ü?not supported automatically. Delete + re-add.
...
## Advanced workflow ‚Ä?when the starter isn't enough
...
Progression: Round 0 design ‚Ü?Round 1 scaffold ‚Ü?Round 2 fill ‚Ü?
Round 2' seed ‚Ü?Round 3 JS. Each round is a deployable state.

### Round 0: System architecture ‚Ä?confirm with user

Write a `DESIGN.md` (markdown, not YAML) covering:

1. **Collections** ‚Ä?every table, its fields, and its relations.
   See `nocobase-data-modeling` skill for field-interface reference.
2. **Page list** ‚Ä?every page, one-line purpose each, grouped by menu.
3. **Navigation wiring** ‚Ä?which m2o fields open which popup
...
```markdown
## Collections
- nb_lib_books (title, author, isbn, category, status, loans: o2m ‚Ü?nb_lib_loans)
- nb_lib_members (name, email, phone, join_date, loans: o2m ‚Ü?nb_lib_loans)
- nb_lib_loans (loan_no, book: m2o, member: m2o, borrowed_at, due_date, returned_at, status)

## Pages (under menu "Library")
- Books list ‚Ä?browse + search books, add new
- Members list ‚Ä?browse members, their loan history
- Loans list ‚Ä?active/overdue loans, return action
- Dashboard ‚Ä?KPIs + charts

## Navigation
- books.table.title ‚Ü?books detail popup (shared template)
- loans.table.book ‚Ü?books detail popup (shared via defaults.yaml)
- loans.table.member ‚Ü?members detail popup (shared via defaults.yaml)
```
...
### Round 1: Scaffold ‚Ä?still start from the starter
...
| Step | What to do |
|---|---|
| Base | `cp -r templates/starter workspaces/<name>` ‚Ä?push once |
...
- `templates/crm/collections/nb_crm_leads.yaml` ‚Ä?collection format
- `templates/crm/routes.yaml` ‚Ä?multi-group routes (shape only)

### Round 2: Fill content ‚Ä?blocks, popups, templates
...
| Create-form with inline sub-table for o2m children | `templates/crm/templates/block/form_add_new_opportunities_quotations_quotations.yaml` ‚Ä?`items` is an o2m field listed in `fields:` and rendered as an inline editable sub-table. Also `templates/crm/pages/main/products/` for master/child UX. |
...
- Copy 10‚Ä?0 lines, adapt names. **Never copy whole files.**
- Don't copy `uid:` / `targetUid:` / `route_id:` ‚Ä?deployer assigns fresh.
...
By default a table's row-action column is empty ‚Ä?NB won't render
...
1. **Is there a boolean / enum status field?** ‚Ü?add `updateRecord`
...
2. **Does the record need a second detail/form view different from
   the default edit popup?** ‚Ü?`popup` with `templateRef`.
3. **Tree/hierarchy collection?** ‚Ü?add `addChild`.
4. **Need to navigate elsewhere with this record's id/filter?** ‚Ü?
   `link` with `url: /admin/...?filter={{ctx.record.id}}`.
5. **Want to let the user clone a complex record?** ‚Ü?`duplicate`.
6. **Need to start a workflow manually?** ‚Ü?`workflowTrigger`.
...
| `updateRecord` | Assign fields + optional linkageRules ‚Ä?the state-change workhorse | `templates/crm/pages/main/overview/layout.yaml` (Done / Undone pair) |
...
| `duplicate` | Clone the record into a new form | ‚Ä?|
| `workflowTrigger` | Manually trigger a workflow on this record | (toolbar in `templates/crm/pages/main/customers/tab_customers/layout.yaml` ‚Ä?same shape works per-row) |
| `historyExpand` / `historyCollapse` | Record-history plugin inline expand | ‚Ä?|
| `ai` | AI employee button (tasks_file + employee) | `templates/crm/pages/main/leads/layout.yaml` (toolbar ‚Ä?same shape per-row) |

Toolbar actions (declared in block-level `actions:`) use the same
type names plus `filter`, `refresh`, `addNew`, `bulkDelete`,
`export`, `import`. Not every type makes sense in both contexts ‚Ä?
`updateRecord` as a toolbar action would apply to *no* specific row,
so put it in `recordActions`.

**Goal of Round 2**: all pages have working CRUD ‚Ä?add / edit / view
...
# Always GET existing record IDs first ‚Ä?they're snowflake integers
...
### Round 3: JS ‚Ä?where CRM uses it, you probably need it
...
|---|---|---|
| **Field renderer / column** (e.g. color-coded status tag, days-until-due badge) | ‚ú?in most list tables | Any field whose display depends on a derived value (date math, status-to-color, multi-field composite) |
| **Block** (KPI card, custom widget inside a form) | ‚ú?overview, analytics, per-form tips | A summary widget, inline chart, or "helper panel" that reads from multiple collections |
| **Dashboard page** (whole page of charts + KPIs) | ‚ú?analytics page | Module has ‚â? measurable metrics users care about; validator **requires** ‚â? charts on pages titled "Dashboard" / "Analytics" |
...
**Dashboards specifically look bad when designed freehand ‚Ä?mirror the
...
| `templates/crm/pages/main/analytics/layout.yaml` | Full dashboard: filterForm row ‚Ü?4 KPI jsBlocks in one row ‚Ü?5 charts in a `16/8 ‚à?full ‚à?14/10` grid. Use when you want ‚â? charts (validator requires this when the page title contains `dashboard` or `analytics`). |

Procedure:
1. Open the reference `layout.yaml`. Note the block keys / types / widths.
2. Write YOUR `layout.yaml` with the SAME shape ‚Ä?same number of blocks,
   same grid widths in the `layout:` section ‚Ä?but your own block
...
| Full-page custom UI (wizard / multi-step / custom flow) | `templates/crm/pages/main/customers/tab_merge/js/customers_merge_jsBlock.js` ‚Ä?whole page is one `type: jsBlock`, ~580 lines React |
...
SQL charts: save + run as a two-step pattern ‚Ä?
...
`key` = lower_snake_ascii identity ‚Ä?drives directory names under `pages/`
...
**`key: reference`** ‚Ä?popup block is a *reference* to the template.
...
**Bare `ref:`** (no `key: reference`) ‚Ä?template content is *inlined*
...
### Auto-created columns ‚Ä?do NOT declare them
...
- m2o / o2m FK columns: declaring `owner: m2o ‚Ü?users` auto-creates
...
### Table vs sub-table ‚Ä?two different things, don't confuse
...
| Use when | Children browsed separately (customer detail ‚Ü?orders list) | Children entered alongside parent (invoice + line items) |

Bare `- tasks` in a form is the third option: a **RecordSelect picker**
("pick existing record"). Rarely what you want ‚Ä?validator warns.
...
(`owner: m2o, foreignKey: owner_id` ‚Ü?`owner_id` on SELF).

On **o2m**, `foreignKey` names the FK column on the *target* table
(`tasks: o2m ‚Ü?nb_pm_tasks, foreignKey: project_id` ‚Ü?`project_id` on
`nb_pm_tasks`).
...
| `string violation` on create | `createdAt`/`updatedAt` declared in YAML ‚Ä?remove |
...
| `Collection X not found in data source main` | `associationName` used a short name ‚Ä?use the full collection name (`nb_pm_projects.tasks`, not `project.tasks`). See `templates/crm/pages/main/customers/tab_customers/popups/` |
| Per-row column shows only edit+delete | `recordActions` missing ‚Ä?see "Per-row actions" below for what to add. Removing an action/field/column from the DSL now *does* destroy it on the NB side on next push. |

---

If any of the above contradicts what you observe at runtime, the manual
is stale ‚Ä?note what was missing and tell the user.
