...
3. 任何探测或写操作前，先按 [references/ops-and-review.md](references/ops-and-review.md) 执行 `start-run`。
4. validation / review / improve 只有在用户明确要求时才进入；未进入浏览器验证时，`browser_attach` / `smoke` 要记为 `skipped`。
