...
18. Resolve filter wording before choosing structure. For table / list / gridCard-like data surfaces, ambiguous “筛选 / filter / 搜索” requests default to a block-level `filter` action, not a new `filterForm` block. Read [references/aliases.md](./references/aliases.md) first. Open [references/blocks/filter-form.md](./references/blocks/filter-form.md) and keep a real `filterForm` in the first-pass blueprint only when the user explicitly asks for a filter block/form/query area; then include stable filter items, `submit` / `reset` actions, and same-blueprint string `target` block keys instead of low-level `defaultTargetUid` or raw block settings payloads.
