...
9. 如果现场 `get/catalog/context` 没有明确暴露目标能力、target 绑定字段、resource binding、settings contract 或 `currentRecord` guard，停止猜测，不要臆造写入；只有文档显式允许的保守 fallback（例如用户明确接受仅保留 popup shell）才能继续。
