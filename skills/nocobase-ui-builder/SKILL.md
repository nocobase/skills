...
- Hard rules before you write:
  ...
  15. For localized edits on an existing template reference, default template-owned content to the template source and keep current host/openView config edits local. Page-scoped wording alone is not local-only intent, so do not auto-detach to `copy`; if live readback still cannot separate template-owned vs host-local scope, clarify before writing. Use [templates.md](./references/templates.md) for the routing matrix.
