...
- Hard rules before you write:
  ...
  15. For localized edits on an existing template reference, default to editing template-owned content at the template source. Keep current host/openView config edits local, and only use `copy` / detach when the user explicitly asks for local-only behavior. Use [templates.md](./references/templates.md) for the routing matrix.
