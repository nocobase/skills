...
18. Resolve filter wording before choosing structure. For table / list / gridCard-like data surfaces, ambiguous “筛选 / filter” requests default to a block-level `filter` action. Treat “搜索 / search” that way only when the request explicitly adds search to a table / list / gridCard / card-like host, including wording such as “支持搜索 / 带搜索 / 可搜索 / searchable”; page-noun wording such as “搜索页 / 搜索结果页 / 搜索门户 / 搜索列表页” should stay page intent, not filter intent. Do not create a new `filterForm` by default. Read [references/aliases.md](./references/aliases.md) first. Open [references/blocks/filter-form.md](./references/blocks/filter-form.md) and keep a real `filterForm` in the first-pass blueprint only when the user explicitly asks for a filter/search block, form, or query area; then include stable filter items, `submit` / `reset` actions, and same-blueprint string `target` block keys instead of low-level `defaultTargetUid` or raw block settings payloads.
19. If a first-pass whole-page write still leaves `filterForm` as an empty shell after a successful `applyBlueprint`, treat it as an explicit local/live gap and keep any low-level `addBlock` / `addAction` / `addField` repair narrowly scoped. If the first `applyBlueprint` fails with a verified `filterForm`-specific shape/runtime error, stop and report the failing blueprint / preview / error evidence instead of switching to low-level writes in the same pre-success phase.
...
23. Use low-level `get-reaction-meta` + `set*Rules` or `add*` repair only for localized edits on an existing live page, or after a successful whole-page `applyBlueprint` when an explicit local/live gap still needs narrowly scoped repair. Before one whole-page `applyBlueprint` succeeds, do not use `createMenu`, `createPage`, `compose`, `configure`, `update-settings`, `add*`, `move*`, `remove*`, or `set*Rules`. If the first whole-page `applyBlueprint` fails, stop and report the failing blueprint / preview / error evidence instead of falling back to low-level writes in that same pre-success phase.
