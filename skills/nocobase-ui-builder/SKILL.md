...
- [normative-contract.md](./references/normative-contract.md) is the single source of truth for blueprint-first planning, `catalog`, popup shell fallback, and schema drift / recovery.
...
8. The default write path for an existing target is `get -> [decide whether to append catalog by normative contract] -> write -> readback`. The only time you may skip one leading `get` is when the next target uid was just returned directly by the previous write API.
...
- [normative-contract.md](./references/normative-contract.md): the single source of truth for blueprint-first planning, `catalog`, popup shell fallback, and schema drift / recovery.
