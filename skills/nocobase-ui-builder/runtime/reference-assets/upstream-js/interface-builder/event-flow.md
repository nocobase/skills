...
When you use this step through the skill, treat the code as RunJS input that must pass the local validator before writeback. Localized persisted payloads are written through `set-event-flow` or `add-event-flow`; full registry replacement uses `set-event-flows`. Do not patch arbitrary raw UI tree props.
