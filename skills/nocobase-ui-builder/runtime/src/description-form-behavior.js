...
export function deriveDescriptionFieldBehavior(fieldMeta, options = {}) {
  ...
  if (!hasOwn(settings, "required")) {
    const required = hasConditionCue
      ? undefined
  ...
  if (!hasOwn(settings, "rules")) {
    const rules = deriveDescriptionValidationRules(fieldMeta, description);
    if (rules.length) {
      settings.rules = rules;
    }
  }
  if (!hasOwn(settings, "maxCount")) {
    const maxCount = deriveMaxCountDescriptionSetting(fieldMeta, description);
