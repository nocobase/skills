...
test('auto session id is stable for the same cwd and ppid', () => {
  const first = createAutoSessionId({ cwd: '/tmp/demo', ppid: 12345 });
  const second = createAutoSessionId({ cwd: '/tmp/demo', ppid: 12345 });
  const other = createAutoSessionId({ cwd: '/tmp/demo', ppid: 54321 });
