...
- Whole-page `create` / `replace` should still probe templates for reusable scenes. A missing live `target.uid` only means template planning must use the strongest planned opener/resource context available.
