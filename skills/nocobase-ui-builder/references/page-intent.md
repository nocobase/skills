...
7. Keep popup behavior inline under the relevant field/action/record action while drafting, but hand any reusable popup / block / fields scene to [templates.md](./templates.md) before treating inline content as final. When no explicit `popup.template` is present, default the popup draft to `popup.tryTemplate=true` on the supported popup-capable write paths, and preserve local popup content as the miss fallback when needed. If the user explicitly wants the new local popup itself to become a reusable popup template immediately, prefer `popup.saveAsTemplate={ name, description }` instead; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
...
- If the user explicitly wants the newly created local popup to become a reusable popup template immediately, prefer `popup.saveAsTemplate={ name, description }` on supported create-time popup writes. That path requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
