...
- Whole-page `create` / `replace` should still probe templates for reusable scenes. For repeat-eligible popup / block / fields scenes, contextual `list-templates` is mandatory before binding one template or finalizing inline content. A missing live `target.uid` only means template planning must use the strongest planned opener/resource context available, and keyword-only search remains discovery-only.
