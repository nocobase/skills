...
7. Keep popup behavior inline under the relevant field/action/record action while drafting, but hand any reusable popup / block / fields scene to [templates.md](./templates.md) before treating inline content as final. When no explicit `popup.template` is present, default the popup draft to `popup.tryTemplate=true` on the supported popup-capable write paths, and preserve local popup content as the miss fallback when needed. If the user explicitly wants the new local popup itself to become a reusable popup template immediately, or if the current whole-page draft already contains the first repeated popup seed with explicit local `popup.blocks` and no usable popup template was found, prefer `popup.saveAsTemplate={ name, description }` instead; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
...
- When the request says a table/list block and a details block both expose relation data and both relation displays need detail popups, treat each same-relation popup family as one same-page repeated relation-details scene even if one opener is a table/display field and the other is a details/read-only field. Probe templates before finalizing those popup field objects.
- For popup-capable writes, when no explicit `popup.template` is present, keep `popup.tryTemplate=true` as the default execution fallback. Local popup content may remain as the miss fallback. This fallback does not replace [templates.md](./templates.md); it only lets the backend attempt the final popup-template match at write time.
- If the user explicitly wants the newly created local popup to become a reusable popup template immediately, or if the page already contains the first repeated popup seed and contextual probing found no usable popup template, prefer `popup.saveAsTemplate={ name, description }` on supported create-time popup writes. That path requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`. Do not assume same-request sibling popups can already bind that fresh template; keep them on `popup.tryTemplate=true`, inline fallback, or a later localized rebind plan.
