...
- For popup-capable writes, when no explicit `popup.template` is present and there is no local popup content, keep `popup.tryTemplate=true` as the default execution fallback. This fallback does not replace [templates.md](./templates.md); it only lets the backend attempt the final popup-template match at write time.
