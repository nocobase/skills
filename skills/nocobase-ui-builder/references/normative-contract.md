...
- may include top-level `defaults.collections` for collection-level generated-popup defaults: fixed `{ name, description }` `popups.view/addNew/edit` for every involved direct collection, fixed `{ name, description }` `popups.associations.<associationField>.view/addNew/edit` for every involved association scope keyed only by the first relation segment, optional `defaults.collections.<collection>.fieldGroups` only when one of those generated popup scenes still has more than 10 effective fields, target-scoped `defaults.collections.<collection>.formBehavior.addNew/edit` for description-derived settings / field linkage rules on backend-generated add/edit forms, and sibling `defaults.collections.<collection>.formBehaviorDescriptionReview = { fields, hasTried: true }` for described generated add/edit fields that were reviewed but not safely converted. Each described generated add/edit candidate field must be covered by structured `formBehavior` or listed once in `formBehaviorDescriptionReview.fields`; if every described generated field falls into the review bucket, `formBehavior` itself may be omitted. Do not use `formBehavior: {}` or `null` as a no-op confirmation. `table` blocks always pull their collection into the `addNew` threshold evaluation
