...
- may set either `navigation.layoutUid` or `navigation.portalUid` in `create`, never both. Omit both for default desktop/admin authoring; use `layoutUid: "mobile-layout-model"` for mobile intent; use `portalUid` only for an explicitly requested workspace returned by `list-navigation-targets`. Mobile layouts and mobile-backed portals keep the visible entry in `navigation.item` and omit `navigation.group`. See [navigation-targets.md](./navigation-targets.md) for target discovery, group resolution, and duplicate-page identity.
