...
- may include top-level `defaults.collections` for collection-level generated-popup defaults: fixed `{ name, description }` `popups.view/addNew/edit` for every involved direct collection, fixed `{ name, description }` `popups.associations.<associationField>.view/addNew/edit` for every involved association scope keyed only by the first relation segment, optional `defaults.collections.<collection>.fieldGroups` only when one of those generated popup scenes still has more than 10 effective fields, and optional target-scoped `defaults.collections.<collection>.formBehavior.addNew/edit` for description-derived settings / field linkage rules on backend-generated add/edit forms; `table` blocks always pull their collection into the `addNew` threshold evaluation
