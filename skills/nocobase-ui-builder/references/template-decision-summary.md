...
For local helper enforcement, this contract is also the normalized `templateDecision` shape checked when optional helper input includes template decision data.

Use the optional helper envelope `{ blueprint, templateDecision?, collectionMetadata? }` only for local planning checks. A valid `templateDecision` may still be returned when other blueprint gates fail after the blueprint is already recognizable, but it must stay semantically consistent with the blueprint's real template bindings. For data-bound blueprints, `collectionMetadata` belongs in helper input or backend metadata context, not in the blueprint root. If the blueprint is not recognizable yet, the helper should omit the normalized summary. If the summary claims `selected-reference` / `selected-copy`, the blueprint must bind at least one matching template uid/mode for that current decision. `discovery-only` / `inline-non-template` only describe that this decision did not bind a template; mixed pages may still contain other bound templates elsewhere. If this consistency check fails, backend authoring validation should surface `inconsistent-template-decision`.
