...
11. If the page only says “增加筛选 / filter” on an existing or requested table/list/gridCard/calendar/kanban-like surface, or explicitly adds “搜索 / search” to that data surface, including wording such as “支持搜索 / 带搜索 / 可搜索 / searchable”, default to the block action slot instead:
   ...
   - direct, non-template public `table` / `list` / `gridCard` / `calendar` / `kanban` blocks must provide an explicit effective `defaultFilter`; backend authoring reports missing filters through aggregate `errors[]` instead of choosing fields
   - keep block-level or filter action `defaultFilter` concrete and metadata-valid; explicit empty groups, invalid operators, relation fields used directly, and unknown paths are rejected through backend aggregate `errors[]`
