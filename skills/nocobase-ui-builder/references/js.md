...
- `Hard rule`：render 类模型统一走 `preview`
- `Hard rule`：action 类模型统一走 `validate`
- `Hard rule`：validator 失败就是失败，不能降级成 warning，也不能继续写入
- `Hard rule`：运行时校验只允许依赖 `./runtime/bin/nb-runjs.mjs`，不允许在执行阶段读取 NocoBase 源码
...
- `Default heuristic`：根据 validator 返回的问题自动重写 code，并重新校验
- `Hard rule`：最多 3 轮；3 轮后仍失败就停止，不要继续 MCP 写入
...
`Fallback`：如果现场能力无法确定是哪一种 JS action，就先停下来，优先读 `catalog` / `get` 收敛容器和上下文，再选 model；不要拍脑袋猜。
...
- `Default heuristic`：如果 live MCP 已知更精确的 `resource` / `collection` / `collectionField` / `record` / `formValues` / `namePath`，就用 live 数据覆盖默认值
- `Hard rule`：不要为了补 context 去读取 NocoBase 源码；只允许使用 runtime profile 和 live MCP 数据
- `Default heuristic`：`network` 默认不传；只有 code 确实依赖读请求且你能提供 mock response 时才传

## Strict Render 规则

以下模型都属于 strict render model：
...
- `Hard rule`：必须使用 `ctx.*` 访问上下文
- `Hard rule`：裸 `record` / `formValues` / `resource` / `collection` / `collectionField` / `value` / `namePath` 都算失败
- `Hard rule`：必须显式调用 `ctx.render(...)`
- `Hard rule`：不能依赖 `return` 自动渲染
- `Hard rule`：如果 validator 报 `bare-compat-access`、`missing-required-ctx-render` 等问题，必须直接修 code，不要绕过
...
- `Hard rule`：任何 JS 写入都必须先通过上面的 RunJS validator gate，再进入 MCP 写流程
