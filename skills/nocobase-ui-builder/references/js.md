...
- Do not translate the phrase "current record" directly into `ctx.record`. Pick a `recordSemantic` first: popup opener data uses `ctx.popup.record`, a row/field host record uses `ctx.record`, parent popup data uses `ctx.popup.parent.record`, and selected table rows use `ctx.resource.getSelectedRows?.()`.
