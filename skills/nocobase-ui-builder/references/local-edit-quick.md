...
9. When the request says “给表格 / 列表 / Grid 增加筛选” and does not explicitly mention a filter block/form, default to `add-action(type="filter")` on that data block. Do not create a `filterForm` shell unless the user explicitly asks for one.
