...
Stay on this route when the request is "change one part of an existing page" rather than "rebuild the whole page". Existing JS owners use [runjs-capability-gate.md](./runjs-capability-gate.md): only a JS Block with one of the two explicit unavailable/unsupported codes and a verified public Host write may use direct `changes.code/changes.version`. A new complete JS Page or JS Block is not an ordinary localized `settings.code` edit: route it through [create-js-page-quick.md](./create-js-page-quick.md). Multiple Hosts sharing one maintained implementation without copied code route through [js-template-source.md](./js-template-source.md); independent Git storage or distribution alone does not override single-Host Inline ownership.
...
7. For popup-capable localized writes, split opener-local config from popup-owned content: `clickToOpen`, outer `openView`, title, size, and mode stay on the opener; popup inner blocks/layout/template routing follow [popup.md](./popup.md) and [ui-templates.md](./ui-templates.md).
...
- [ui-template-quick.md](./ui-template-quick.md) if the live target already carries a template reference
