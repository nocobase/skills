...
11. When the request says “给表格 / 列表 / Grid / gridCard / 日历 / calendar / 看板区块 / kanban 增加筛选”, or explicitly adds search to a table / list / Grid / gridCard / calendar / kanban / card-like host, including “支持搜索 / 带搜索 / 可搜索 / searchable”, and does not explicitly mention a filter/search block or form, default to that existing data block's `filter` action. Do not create a `filterForm` shell unless the user explicitly asks for one. If the localized write creates a new direct non-template public `table` / `list` / `gridCard` / `calendar` / `kanban` block through `compose`, `add-block`, or `add-blocks`, omitted `defaultFilter` is materialized by backend authoring from live metadata with 4 scalar/filterable fields. Provide explicit block-level `defaultFilter` only when overriding those generated fields. Explicit empty, invalid, relation-field, unknown-path, or fewer-than-4-field `defaultFilter` is rejected through backend aggregate `errors[]`; for relation filters use a child path such as `department.title`, not `department` itself. For every direct public data surface, partial `actions` complete to that host's defaults (`filter` / `refresh` / `addNew`, plus table `bulkDelete`), and table partial `recordActions` complete to `view` / `edit` / `delete`. If both block-level `defaultFilter` and filter action `settings.defaultFilter` exist, the action-level one wins. Page-noun wording such as “搜索页 / 搜索结果页 / 搜索门户” should not take this path by default, even if the same sentence also says “支持搜索”.
