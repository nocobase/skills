...
14. When you locally author one localized `compose` / `add-block` / `add-blocks` / `configure` body, validate it with the local localized preflight helper (`node skills/nocobase-ui-builder/runtime/bin/nb-localized-write-preflight.mjs --operation <compose|add-block|add-blocks|configure> --stdin-json`) or `runLocalizedWritePreflight(...)` before the later explicit `nb api flow-surfaces ...` call. Use caller-supplied `collectionMetadata`, keep the helper fail-closed on missing metadata or missing/empty block-level `defaultFilter` for direct non-template public data-surface blocks, and for localized `configure` provide `collectionMetadata.liveTopology.byUid[target.uid]` when you want host-aware validation or normalization against a known live target. Keep the detailed helper contract in [helper-contracts.md](./helper-contracts.md). After a successful helper run send `result.cliBody` in that explicit nb write. This helper validates local payload shape; it does not wrap or execute the nb transport for you.
