...
- Dashboard chart sections must stay as chart blocks in whole-page payloads. Use `assets.charts.<key>` plus `{ "type": "chart", "chart": "<key>" }`; do not replace requested chart / 图表 / trend / distribution / ranking sections with `jsBlock`, `table`, or `list`.
