...
- 无论是否真的创建 `filter` action，只要是 public `table` authoring（`applyBlueprint` / `compose` / `add-block` / `add-blocks`），区块本身都必须带非空 block-level `defaultFilter`。
...
- `settings.sort` is only a compatibility alias; prepare-write and localized preflight normalize it to `settings.sorting` for sortable public blocks.
