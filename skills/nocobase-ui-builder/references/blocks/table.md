...
- public `table` authoring（`applyBlueprint` / `compose` / `add-block` / `add-blocks`）必须提供显式有效的 block-level `defaultFilter` 或等效 filter action `settings.defaultFilter`；后端不会替 UI Builder 选择字段，缺失、空组、非法 operator、关系字段本身或未知字段路径都会通过 aggregate `errors[]` 返回。关系筛选写下一级字段，例如 `department.title`，不要写 `department` 本身。
