...
- 新建表格时必须同时给默认筛选设置。
- Whole-page / `compose` authoring 使用 `actions[]` 里的对象 `filter` action，带 `settings.filterableFieldNames` 和 `settings.defaultFilter`。
- 默认筛选字段从目标 collection 里选 3 到 4 个常用字段；字段不足时用现有字段。优先 `username` / `nickname` / `name` / `title` / `email` / `status` / `type` / `category`，避免 `id`、审计字段、密码、token、secret 和大文本。
- 文本字段用 `$includes`；枚举、布尔、数字、日期、关系字段用 `$eq`；`value` 可以为空字符串；`defaultFilter.items` 必须覆盖所有 `filterableFieldNames`。
