...
6. 如果用户明确要自定义 / 插件 / vendor / 未知区块，先读 [../dynamic-capabilities.md](../dynamic-capabilities.md)。内置已记录区块仍优先走本索引里的专用文档；动态能力只使用 `publicType` / public `type`，写入前必须由 target-scoped `catalog` 确认，且不能请求 `debugImplementation` 或把 `capabilityId` / internal keys 写入 payload。
