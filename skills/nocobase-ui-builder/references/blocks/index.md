...
6. 如果用户明确要自定义 / 插件 / vendor / 未知区块，先读 [../dynamic-capabilities.md](../dynamic-capabilities.md)。内置已记录区块仍优先走本索引里的专用文档；动态能力只使用 `publicType` / public `type`，写入前必须由 target-scoped `catalog` 确认，必要时再用 `describeCapability` + `validateCapabilityCreate` 校验 public payload；`capabilities` / `supportLevel` 不能单独授权写入，不能请求 `debugImplementation`，也不能把 `capabilityId` / `modelUse` / implementation `use` / `props` / `decoratorProps` / `stepParams` / `flowRegistry` / `createModelOptions` / `defaultNode` / `lens` 或 `dryRunNode.use` 写入 payload。
