...
- direct non-template public `kanban` authoring 的 `defaultFilter` 必须显式有效，并默认把筛选/搜索落到同一个 kanban host 的 block-level `filter` action；backend authoring 不会替 UI Builder 选择字段，缺失、空组、非法 operator 或未知字段路径都会通过 aggregate `errors[]` 返回
