...
如果 `applyBlueprint` 的 direct non-template `kanban` 省略 `fields[]`，helper 会从 live metadata 自动选择最多 2 个适合卡片展示的字段：必须有非空 `interface`，排除 `id`、audit、hidden、primaryKey、sort、当前 `groupField` 和 `dragSortBy`，并优先标题字段或业务可读字段。调用者显式给出 3 个或更多字段时，本地和后端都返回 `kanban-main-fields-too-many`；不要自动裁剪显式字段。

## 拖拽排序

- `applyBlueprint` direct non-template `kanban` 默认 `settings.dragEnabled=true`
- 如果 live metadata 里已有和当前 `groupField` 兼容的 `interface=sort` 字段，helper 会发送 `settings.dragSortBy`
- 如果没有兼容 sort 字段，helper 省略 `dragSortBy`，后端会在可写 main datasource 集合上自动创建 hidden sort 字段并绑定
