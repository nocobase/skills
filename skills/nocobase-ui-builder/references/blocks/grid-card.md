...
- public `gridCard` authoring（`applyBlueprint` / `compose` / `add-block` / `add-blocks`）可以省略 `defaultFilter`；后端会根据 live metadata 自动生成 4 个标量/可筛选字段。只有需要覆盖默认字段时才显式写 `defaultFilter`；显式空组、非法 operator 或未知字段路径都会通过 aggregate `errors[]` 返回。
