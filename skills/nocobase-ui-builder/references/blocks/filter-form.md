...
- 如果首次 whole-page `applyBlueprint` 失败，停止并报告 blueprint / preview / error 证据；不要在同一个 pre-success phase 改走低层写
...
- 如果 single-shot whole-page 失败，停止并报告 blueprint / preview / error 证据；不要把 staged 低层写法当默认主流程或同阶段兜底
