...
4. 如果是“当前记录下的关联子表”，只有在 parent->child relation resource 已被验证时才优先使用 `resourceSettings.init.associationName + sourceId`；未验证前允许保留 child-side 的逻辑 `dataScope.filter`
5. `associationName` 不能只靠子表上 `belongsTo(parent)` 的字段名猜；`order` 和 `order_items.order` 这类 child-side 写法都不算已验证资源协议
...
| popup page 内继续挂子表 | 若 parent->child resource 已验证，则用 `sourceId = {{ctx.view.inputArgs.filterByTk}}` 配合 `associationName`；否则保留 child-side 的逻辑 relation filter |
