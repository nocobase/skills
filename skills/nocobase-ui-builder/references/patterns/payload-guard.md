...
11. `audit-payload` 报 blocker 时默认停止写入
12. 如果上层任务显式要求某个动作能力，例如“某个 collection 必须有记录级编辑对话框”，要把要求作为 `requirements` 传给 guard
