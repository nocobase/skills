...
- 需要真正落库时，先用 MCP 拿到 write/readback artifact，再交给 wrapper；不要自己手动接上底层写接口
...
24. flowPage v2 的 `RootPageModel` 只能走 page anchor：不要把页面 route `schemaUid` 直接当 `RootPageModel.uid`，也不要把 visible tabs 持久化到 `RootPageModel.subModels.tabs`
25. flowPage v2 的显式可见 tabs 必须先有 child desktopRoutes；真正持久化的入口是各自 `parentId=<tabSchemaUid>, subKey=grid`
...
7. 真正落库时，把上面的 payload/metadata/requirements 连同 write/readback artifacts 一起交给 `ui_write_wrapper.mjs`；不要手动继续裸写
...
- `ROOT_PAGE_TABS_ROUTE_DRIVEN_ONLY`
- `ROOT_PAGE_DIRECT_ROUTE_UID_WRITE_BLOCKED`
- `ROOT_PAGE_ANCHOR_LOCATOR_INVALID`
