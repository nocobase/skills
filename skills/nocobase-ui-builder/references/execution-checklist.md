...
- If a localized edit already hits a template reference, resolve [templates.md](./templates.md) before writing. Default template-owned content to the template source, treat page-scoped wording as insufficient for local-only intent, and require an explicit local-only cue before any `convert-template-to-copy`.
