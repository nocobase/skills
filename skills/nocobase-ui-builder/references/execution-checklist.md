...
5. When the task enters a reusable popup / relation-click / fields-template scene, route template choice through [templates.md](./templates.md) before committing to inline popup or duplicated field content. For repeat-eligible scenes, run contextual `list-templates` first; keyword-only search is discovery-only. When no explicit `popup.template` is present, default that popup-capable write to `popup.tryTemplate=true`, and keep local popup content as the miss fallback when it exists. If the user explicitly wants the new local popup to become a reusable popup template immediately, prefer `popup.saveAsTemplate={ name, description }`; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
