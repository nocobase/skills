...
- If a localized edit already hits a template reference, resolve [templates.md](./templates.md) before writing so you choose between template-source edit, host-local config edit, popup-template switch, and explicit detach-to-copy. Do not default to `copy`.
...
5. When the task enters a reusable popup / relation-click / fields-template scene, or when the current target already carries a template reference, route through [templates.md](./templates.md) before writing. For repeat-eligible scenes, run contextual `list-templates` first; keyword-only search is discovery-only. For existing references, decide explicitly between template-source edit, host-local config edit, popup-template switch, and explicit detach-to-copy. When no explicit `popup.template` is present, default that popup-capable write to `popup.tryTemplate=true`, and keep local popup content as the miss fallback when it exists. If the user explicitly wants the new local popup to become a reusable popup template immediately, prefer `popup.saveAsTemplate={ name, description }`; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
