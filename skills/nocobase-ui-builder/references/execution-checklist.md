...
11. Unless follow-up operations or explicit inspection are needed, stop after successful `apply-blueprint`. Only when the next step needs live structure should you use the returned `target` / `pageSchemaUid` for `nocobase-ctl flow-surfaces get` and targeted readback from [verification.md](./verification.md).
