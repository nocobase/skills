...
- If the task contains reusable or repeated popup / block / fields scenes, read [templates.md](./templates.md) before deciding inline vs template authoring. Whole-page `create` / `replace` is not exempt; when no live host/opener exists yet, use the strongest planned scene context you already have. For repeat-eligible scenes, run contextual `list-templates` probing first and treat keyword-only search as discovery-only.
