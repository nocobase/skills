...
- When the user explicitly wants the new local popup itself to become a reusable popup template immediately, plan to send `popup.saveAsTemplate={ name, description }` on those same create-time popup write paths instead of a separate save step. This requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
