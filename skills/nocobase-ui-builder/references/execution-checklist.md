...
- If the task contains reusable or repeated popup / block / fields scenes, or one single standard reusable scene with strong context, read [templates.md](./templates.md) before deciding inline vs template authoring. Whole-page `create` / `replace` is not exempt; when no live host/opener exists yet, use the strongest planned scene context you already have. For repeat-eligible and strong-context standard reusable scenes, run contextual `list-templates` probing first and treat keyword-only search as discovery-only.
- If a localized edit already hits a template reference, resolve [templates.md](./templates.md) before writing. Default template-owned content to the template source, treat page-scoped wording as insufficient for local-only intent, and require an explicit local-only cue before any `convert-template-to-copy`. If the requested change is popup-owned content under an existing referenced popup subtree, keep it on the template-source/localized route instead of switching route families as a shortcut.
- When no explicit `popup.template` is present, plan to send `popup.tryTemplate=true` on popup-capable `add-field` / `add-fields`, `add-action` / `add-actions`, `add-record-action` / `add-record-actions`, `compose` action/field popup specs, and whole-page `applyBlueprint` inline popup specs. Local popup content may remain as the miss fallback. This is a write fallback only; [templates.md](./templates.md) still owns the planning decision.
- When the user explicitly wants the new local popup itself to become a reusable popup template immediately, or when the current task already contains the first repeated popup seed with explicit local `popup.blocks` and contextual probing found no usable popup template, plan to send `popup.saveAsTemplate={ name, description }` on those same create-time popup write paths instead of a separate save step. This requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`. Do not assume later sibling popups in the same request can already bind that fresh template.
...
- rewriting popup-owned content under an existing template reference; route through template-source or host-local localized edits first
...
7. If the draft contains a reusable popup / block / fields scene, hand template planning off to [templates.md](./templates.md) before finalizing inline content. For repeat-eligible scenes, contextual `list-templates` is mandatory before binding a template or finalizing the inline fallback. Use live context when present; otherwise use the strongest planned opener/resource scene context. When no explicit `popup.template` is present, keep `popup.tryTemplate=true` in the draft as the default popup write fallback, and preserve any local popup content as the miss fallback when needed. If the user explicitly wants that new local popup to be kept as a reusable popup template immediately, or if the first repeated popup seed is already explicit local popup content and no usable popup template exists yet, use `popup.saveAsTemplate={ name, description }` instead; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
...
5. When the task enters a reusable popup / relation-click / fields-template scene, or when the current target already carries a template reference, route through [templates.md](./templates.md) before writing. For repeat-eligible scenes, run contextual `list-templates` first; keyword-only search is discovery-only. For existing references, keep the bridge thin here: template-owned edits default to the template source, host/openView config stays local, page-scoped wording alone is not enough for `copy`, and unresolved scope should be clarified instead of auto-detaching. If that existing reference owns popup content, do not rewrite it through whole-page `applyBlueprint replace`, new inline popup content, or `popup.tryTemplate=true`. When no explicit `popup.template` is present on a new popup-capable write, default that write to `popup.tryTemplate=true`, and keep local popup content as the miss fallback when it exists. If the user explicitly wants the new local popup to become a reusable popup template immediately, prefer `popup.saveAsTemplate={ name, description }`; it requires explicit local `popup.blocks` and cannot be combined with `popup.template` or `popup.tryTemplate`.
