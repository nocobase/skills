...
- When no explicit `popup.template` is present and there is no local popup content, plan to send `popup.tryTemplate=true` on popup-capable `add-field` / `add-fields`, `add-action` / `add-actions`, `add-record-action` / `add-record-actions`, `compose` action/field popup specs, and whole-page `applyBlueprint` inline popup specs. This is a write fallback only; [templates.md](./templates.md) still owns the planning decision.
...
7. If the draft contains a reusable popup / block / fields scene, hand template planning off to [templates.md](./templates.md) before finalizing inline content. For repeat-eligible scenes, contextual `list-templates` is mandatory before binding a template or finalizing the inline fallback. Use live context when present; otherwise use the strongest planned opener/resource scene context. When no explicit `popup.template` is present and there is no local popup content, keep `popup.tryTemplate=true` in the draft as the default popup write fallback.
...
5. When the task enters a reusable popup / relation-click / fields-template scene, route template choice through [templates.md](./templates.md) before committing to inline popup or duplicated field content. For repeat-eligible scenes, run contextual `list-templates` first; keyword-only search is discovery-only. When no explicit `popup.template` is present and there is no local popup content, default that popup-capable write to `popup.tryTemplate=true`.
