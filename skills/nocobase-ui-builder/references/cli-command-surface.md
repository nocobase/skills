...
- In CLI flows, those objects are usually passed through `--body` or `--body-file`. For the first whole-page write after `prepare-write`, that object is the prepared `cliBody`, not the original draft blueprint.
