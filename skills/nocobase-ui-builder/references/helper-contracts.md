...
- when metadata is available and the collection popup descriptors exist, prepare-write auto-generates missing collection-level `defaults.collections.<collection>.fieldGroups` for large generated popups only when that `fieldGroups` key is absent; caller-supplied `fieldGroups` remain authoritative, and malformed provided values still fail locally instead of being overwritten
