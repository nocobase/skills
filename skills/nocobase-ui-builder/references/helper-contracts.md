...
- relation field display objects stay ID-safe: when the target collection's effective `titleField` resolves to `id` (either metadata explicitly sets `titleField = "id"` or omits it and falls back to `id`), prepare-write first auto-fills a readable `titleField` such as `name`, `title`, or `code` when metadata exposes one; explicit `titleField: "id"` is rejected, and prepare-write fails closed when the target exposes no readable display field
...
- relation field display objects keep the same ID-safe guard here too: if the target collection's effective `titleField` resolves to `id` (either metadata explicitly sets `titleField = "id"` or omits it and falls back to `id`), localized preflight first auto-fills a readable `titleField` when metadata exposes one; explicit `titleField: "id"` is rejected, and localized preflight fails closed when the target exposes no readable display field
