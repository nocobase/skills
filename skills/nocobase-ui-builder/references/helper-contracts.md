...
Use this contract for the internal gate that runs before the first real whole-page write.
...
- pass wrapper flag `--no-auto-collection-metadata` to keep fail-closed behavior; then any data-bound block (a block with `collection`, `resource`, `binding`, `dataSourceKey`, `associationPathName`, or `associationField`) with missing or empty metadata fails with `missing-collection-metadata`
...
- relation field objects with explicit `fieldType` also stay strict: when the target collection's effective `titleField` resolves to `id` (either metadata explicitly sets `titleField = "id"` or omits it and falls back to `id`), the field object must explicitly set its own `titleField`; a readable field such as `name`, `title`, or `code` is recommended when available, and explicit `titleField: "id"` remains allowed when intentional
...
- relation field objects with explicit `fieldType` keep the same explicit-title guard here too: if the target collection's effective `titleField` resolves to `id` (either metadata explicitly sets `titleField = "id"` or omits it and falls back to `id`), the localized field object must explicitly set `titleField`; a readable field such as `name`, `title`, or `code` is recommended when available, and explicit `titleField: "id"` remains allowed when intentional
