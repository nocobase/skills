...
- accepts omitted `table` / `list` / `gridCard` / `calendar` / `kanban` `filter` actions, but direct non-template public data surfaces must carry an explicit effective `defaultFilter`; the backend does not choose filter fields. Missing, explicit `{}`, `null`, `{ logic: "$and", items: [] }`, invalid operators, relation fields used directly, and unknown paths are rejected through aggregate `errors[]`; use scalar fields or relation child paths such as `department.title`. Explicit `filterableFieldNames` are checked against filter action `settings.defaultFilter` when present; otherwise use `defaultActionSettings.filter.defaultFilter` or block-level `defaultFilter`.
...
- direct non-template public `table` / `list` / `gridCard` / `calendar` / `kanban` blocks must provide an explicit effective `defaultFilter`; backend authoring returns aggregate `errors[]` instead of materializing a guessed filter when it is missing
- metadata-aware `defaultFilter` checks now belong to backend authoring: missing filters, empty groups, unknown paths, relation fields used directly, invalid operators, non-filter action `settings.defaultFilter` / `filterableFieldNames`, and explicit `filterableFieldNames` coverage mismatches return aggregate `errors[]`. Historical `defaultActionSettings.filter.defaultFilter` inputs remain compatibility-normalized where supported; prefer block-level or filter action settings in new public payloads.
