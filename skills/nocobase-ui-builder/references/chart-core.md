...
除此之外，chart block 只额外暴露 4 个区块外层参数；详细规则统一看下文“区块外层参数（最小暴露集）”。
...
- `title` 只接受非空字符串；`displayTitle` 只接受 `true | false`
- `height` 只接受数字；通常与 `heightMode = "specifyValue"` 搭配
...
任何 `visual.raw` 写入都必须先按 [js.md](./js.md) 走 validator gate，并使用 `ChartOptionModel`；CLI 与 network 约束统一看 [runjs-runtime.md](./runjs-runtime.md)。
...
任何 `events.raw` 写入都必须先按 [js.md](./js.md) 走 validator gate，并使用 `ChartEventsModel`；CLI 与 network 约束统一看 [runjs-runtime.md](./runjs-runtime.md)。
...
- `cardSettings` 是主判据；`tree.decoratorProps.*` 若存在只作辅助镜像，`tree.props.*` 不是主判据
