...
if (-not (Test-Path -LiteralPath $ctlWrapper -PathType Leaf)) {
  Record-Check fail 'CLI-001' "Cannot find skill-local ctl wrapper: $ctlWrapper" 'Ensure nocobase-env-bootstrap/scripts/run-ctl.mjs exists, then rerun postcheck.'
  Write-Host "summary: fail=$Fail warn=$Warn pass=$Pass"
  exit 1
}
...
if ($Fail -eq 0) {
  ...
  if ($readbackResult.ExitCode -ne 0) {
    Record-Check fail 'CLI-005' "Readback failed: $($readbackResult.OutputText)" 'Run `node ./scripts/run-ctl.mjs -- env -s <scope>` manually and verify.'
  } else {
    ...
    if ($text -match [regex]::Escape($EnvName) -and $text -match [regex]::Escape($resolvedBaseUrl)) {
      ...
      Record-Check warn 'CLI-005' 'Readback completed but expected env/base URL was not clearly found in output.' 'Inspect `node ./scripts/run-ctl.mjs -- env -s <scope>` output manually.'
    }
  }
}
