...
Preflight must run with explicit `install_method` context (`<SKILL_ROOT>` = directory of SKILL.md):

```bash
node "<SKILL_ROOT>/scripts/preflight.mjs" --port <port> --install-method <docker|create-nocobase-app|git> --db-mode <bundled|existing> --db-dialect <postgres|mysql|mariadb> --db-database-mode <existing|create>
```
...
- For fresh install targets where project files do not exist yet, missing `APP_KEY` can be deferred and generated by `scripts/install.mjs`.

7. CLI bootstrap prerequisites
- Verify `nb` is available in PATH.
...
- `Use $nocobase-plugin-manage enable <activation_plugin_bundle> -> restart app -> rerun MCP validation`

2. API key activation (API-key mode)
- Verify token env var exists (default `NOCOBASE_API_TOKEN`).
- Probe endpoint with bearer token.
- For `401/403`, use CLI token refresh path (`nocobase generate-api-key`) first when available.
...
- Node failures are blockers for all install methods (`docker`, `create-nocobase-app`, `git`) because skill runtime scripts are Node-based.
- Yarn failures remain blockers for `install_method=create-nocobase-app|git`.
