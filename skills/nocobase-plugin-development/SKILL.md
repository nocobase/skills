...
| `addCollection` (client-side) | Do NOT add unless needed | Only when the collection must appear in block picker |
...
2. **Collection not showing in block picker** → Need client-side `addCollection` with `filterTargetKey: 'id'`.
